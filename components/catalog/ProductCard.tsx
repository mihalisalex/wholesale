...
export function ProductCard({ product }: { product: Product }) {
  return (
    ...
    >
      <div className="relative p-6" style={{ background: `linear-gradient(160deg, ${product.colorHex}14, #fff)` }}>
        ...
        <ProductLineArt art={product.images[0]?.art as ProductArtKey} className="w-full h-28 text-ink" />
      </div>
      <div className="p-5">
        <p className="text-[0.68rem] uppercase tracking-wide text-ink/40 mb-1">{product.collection}</p>
        <h3 className="font-serif text-base font-semibold mb-1 group-hover:text-gold transition-colors">{product.name}</h3>
        <p className="text-xs text-ink/55 mb-3">{product.material}</p>
        <div className="flex items-center justify-between text-sm">
          <span className="flex items-center gap-1.5 text-ink/60">
            <span className={`w-1.5 h-1.5 rounded-full ${STOCK_DOT[product.stockStatus]}`} />
            {STOCK_LABEL[product.stockStatus]}
          </span>
          <span className="font-serif font-semibold">{formatCurrency(product.pricePerPackage)}</span>
        </div>
